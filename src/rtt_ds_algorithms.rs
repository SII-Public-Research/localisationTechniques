...
use crate::{error::Error, ok_or_panic, sync::RpiSync, tools::*, uwb_basics::*};
...
use std::{marker::Send, thread, time::Duration};
...
) -> Result<UWBSensor<SPI, CS, Ready>, (UWBSensor<SPI, CS, Ready>, &'static str)>
where
    ...
    CS: OutputPin + Send + 'static,
{
    ...
    let mut sensor = match sensor.uwb_receive(timeout, &mut data_anch1) {
        Ok(mut sensor) => {
            ...
            if data_anch1.data[0] == 1099511627775 {
                // 2^40 - 1
                sensor.error = 1;
            }
            sensor
    ...
    sensor = ok_or_panic(calc_distance_double(sensor), "Distance calculation failed");
    sensor = ok_or_panic(iir_filter(sensor), "Filter failed");
    ...
    CS: OutputPin + Send + 'static,
{
    ...
    let delay_rx_tx = (sensor.timing_data[1] + (15000 * 63898)) % 1_0995_1162_7776_u64;
    ...
    CS: OutputPin + Send + 'static,
{
    ...
    /* SYNC */
    if let Err(err) = sync.sync() {
        return Err((sensor1, sensor2, sensor3, err));
    }
    ...
    println!(
        "Temps d'arrivées: [1] {:?} ; [2] {:?} ; [3] {:?}",
        sensor1.timing_data.clone(),
        sensor2.timing_data,
        sensor3.timing_data
    );
    ...
    sensor1 = ok_or_panic(iir_filter(sensor1), " Distance filter anchor 01 failed");

    sensor2 = ok_or_panic(
        calc_distance_double(sensor2),
        "Distance calculations anchor 2 failed",
    );
    sensor2 = ok_or_panic(iir_filter(sensor2), " Distance filter anchor 01 failed");

    sensor3 = ok_or_panic(
        calc_distance_double(sensor3),
        "Distance calculations anchor 3 failed",
    );
    sensor3 = ok_or_panic(iir_filter(sensor3), " Distance filter anchor 01 failed");
    ...
    println!(
        "Anchor 1 - Filtered distance = {}",
        sensor1.filtered_distance
    );
    println!(
        "Anchor 2 - Filtered distance = {}",
        sensor2.filtered_distance
    );
    println!(
        "Anchor 3 - Filtered distance = {}",
        sensor3.filtered_distance
    );
    ...
    CS: OutputPin + Send + 'static,
{
    ...
    let mut uwb_data_r = UwbRData {
        // structure dans laquelle sont stockées les données de la fonction uwb_reception
        addr_sender: Some(dw3000::mac::Address::Short(
            dw3000::mac::PanId(0x111),
            dw3000::mac::ShortAddress(0x0),
        )),
    ...
    let delay_rx_tx = (sensor.timing_data[3] + (100000 * 63898)) % 1_0995_1162_7776_u64;
    sensor.timing_data[4] = ((delay_rx_tx >> 9) << 9) + ant_delay;
