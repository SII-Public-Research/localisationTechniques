...
fn main()
{
    ...
    loop {
        ...
        uwbsensor = ok_or_panic(uwbsensor.uwb_send(&[0], None), "fail");
        println!("TIMESTAMP AVANT SYNC : {}", ok_or_panic(uwbsensor.dw3000.ll().tx_time().read(), "fail").tx_stamp());
        sync.sync();

        uwbsensor = ok_or_panic(uwbsensor.uwb_send(&[0], None), "fail");
        println!("TIMESTAMP APRES SYNC : {}", ok_or_panic(uwbsensor.dw3000.ll().tx_time().read(), "fail").tx_stamp());
